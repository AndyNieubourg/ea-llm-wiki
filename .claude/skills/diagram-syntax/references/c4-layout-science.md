...
> Adapted from tractorjuice/arc-kit, `plugins/arckit-claude/skills/mermaid-syntax/references/c4-layout-science.md` (MIT). The graph drawing science and citations are the original; the notation-choice guidance follows this wiki's own rule.

A research-backed reference for creating readable C4 architecture diagrams with minimal edge crossings and clear visual hierarchy. It carries the graph drawing science, layout optimisation techniques, and quality targets behind the syntax references.
...
4. **Switch to PlantUML** if Mermaid's layout cannot achieve acceptable results after the first three steps (see Section 7 for the choice rule and the directional hints)
...
- **Line breaks**: Use `<br/>` for node labels in all diagram types. `<br/>` also works in a quoted flowchart **edge** label: `A["A"] -->|"Uses<br/>HTTPS"| B["B"]` renders as a two-line edge label on `mmdc` 11.15.0, verified by render, and the unquoted form produces a byte-identical PNG. Obsidian's own bundled Mermaid build was not tested, so a page that depends on a multi-line edge label needs an Obsidian render before it ships

---

## 7. PlantUML Directional Hints

> **Cross-reference**: For the full C4-PlantUML syntax reference including element macros, boundary syntax, layout conflict rules, and worked examples, see the PlantUML references in this skill under `references/plantuml/`. This section provides a summary of directional hints only.

PlantUML with the C4-PlantUML library provides directional relationship hints that give explicit control over layout, which Mermaid does not offer. That is the real capability gap between the two renderers: Mermaid gives you declaration order and nothing else, so edge crossings are steered indirectly, while PlantUML lets you place a relationship and pin an element outright.
...
### Which track draws this diagram

Notation decides, and the decision is not made here. The single routing table in this skill is `SKILL.md` section 1, and it restates the project `CLAUDE.md`, which wins on any divergence. Element count does not route a diagram: it measures how much layout work the diagram needs once the track is already fixed.
...
| Too many elements (more than 15) | Exceeds Mermaid's Dagre engine capacity for readable layout; crossings multiply exponentially | Split into multiple diagrams at natural boundaries. Splitting is the fix. Switching tool is not, because the track is already fixed by notation |
...
| Assuming `<br/>` breaks a flowchart edge label | It does not on `mmdc` 11.15.0. `-->\|"Uses<br/>HTTPS"\|` renders as a clean two-line label, verified by render; the unquoted form gives a byte-identical PNG. The "Parse error" claim is false on the pinned renderer | Use `<br/>` when a two-line edge label reads better. Obsidian's bundled Mermaid build was not tested, so check such a page with an Obsidian render |
...
7. **Document accepted trade-offs.** If a crossing cannot be eliminated without sacrificing clarity elsewhere, say so in the prose around the diagram.
...
- Edge labels use comma-separated text, a style choice rather than a constraint. `<br/>` renders in an edge label on `mmdc` 11.15.0 (see section 6)
...
*Section 7 gives PlantUML the C4 work by notation rule. The Mermaid examples above stay useful for the diagrams Mermaid owns and for reading C4-shaped Mermaid already in the vault.*
