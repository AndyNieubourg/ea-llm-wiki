...
The final document must sound like the user extended their own thinking, not like a generic LLM rendering of it. **Read `wiki/_resources/voice-guide.md` before drafting — it is the authoritative, self-contained voice anchor for this wiki.** Its stylometric fingerprint and worked examples carry the register directly, so no external files need reading. It explicitly warns against anchoring on the wiki's existing `analyses/` pages, which are Claude-drafted and carry an over-essayistic register.

**Headline conventions (full detail in the guide):**

- **Business-deliverable structure, not essay structure.** Headings + bolded labels + bullets + tables. The natural unit is the bullet or table row, not the paragraph. Paragraphs appear when an argument actually needs them.
- **Audience-aware sections.** Translate the analysis into next-move instructions for a named downstream consumer (the owner's named readers: CIO/CTO, governance council, board, account manager, delivery team, evaluator).
- **Concrete artifacts, named tooling, named thinkers.** Name the actual tools, methods (by version/article where it matters), and authors. If a thing has a name, name it.
- **Active verbs leading the bullet.** "Led", "Owned", "Coordinated", "Established". Anti-passive.
- **Comparison tables when comparing.** Multi-dimension tables for two-plus things sitting alongside each other.
- **Owner's locale / regulatory context where it matters.** Regulation by article (e.g. EU AI Act, GDPR, DORA). Defer to the locale defaults set in the owner profile ([[CLAUDE]]) and the voice guide.
- **Currency tags** ("Status: open", "Second submission", "Carried forward from prior lint", "NEW 2026-05-24").

**Do not:** Academic / essayistic register. Theory-speak that loses contact with practice ("performative enactment", "epistemic constitution"). Buzzword salad ("synergistic", "leverage", "best-of-breed", "world-class"). Hedging without reason. Bullet-point dumps without spine. Motivational closing paragraphs. Rhetorical questions that imply their own answer. Over-claiming experience.

**Test:** Would the author actually write this in a CV rationale, a briefing memo, or an RFP response — to a downstream consumer who has to act on it? If not, rewrite as a direct claim. If a sentence would survive being copy-pasted into any consultancy deck, cut it.
